...
class Hyperliquid:
  ...
  def http(
    ...
    public: bool = False, base_url: str | None = None,
    ...
    if wallet is None:
      ...
      exchange = Exchange.http(
        wallet, mainnet=mainnet, validate=validate, http=http,
        base_url=base_url,
      )
    return cls(
      info=Info.http(
        mainnet=mainnet, validate=validate, http=http, base_url=base_url,
      ),
  ...
  @classmethod
  def ws(
    cls, wallet: Wallet | None = None, *, mainnet: bool = True,
    validate: bool = True, public: bool = False, ws_url: str | None = None,
  ):
    ...
    ws = SocketClient(url=ws_url or f'wss://{domain}/ws')
    wallet = _parse_wallet(wallet, mainnet=mainnet)
    if wallet is None:
      if not public:
        raise ValueError(
      ...
      exchange = Exchange.ws_of(
        wallet, ws=ws, mainnet=mainnet, validate=validate,
      )
    return cls(
